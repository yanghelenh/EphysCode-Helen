% visstimEphysOptoVInj.m
%
% Experimental Function
% For simultaneous acquisition of
...
%
% UPDATED:
%   2/22/24 - HHY - modification of visstimLegFictracEphysOptoVInj
%

function [rawData, inputParams, rawOutput] = visstimEphysOptoVInj(...
    settings, duration)
    ...
    % name of trial type
    inputParams.exptCond = 'visstimEphysOptoVInj';
    ...
    inputParams.dInCh = {'HgLampShutterSyncOut'};
    ...
    if (oStimTypeFileName == 0)
        ...
        error('No optogenetic stimulation protocol was run. Ending visstimEphysOptoVInj()');
    end
    ...
    try
        ...
        % rethrow(errMes);
        error('Invalid optogenetic stimulation function. Ending visstimEphysOptoVInj()');
    end
    ...
    if (contains(inputParams.visstimMode, 'closedLoop'))
       if (contains(inputParams.visstimMode, 'X'))
            ...
            if (vInjTypeFileNameX == 0)
                % throw error message; ends run of this function
                error('No voltage injection protocol was run. Ending visstimEphysOptoVInj()');
            end
            ...
            try
                ...
                rethrow(errMes);
                % error('Invalid voltage injection function. Ending visstimEphysOptoVInj()');
            end
