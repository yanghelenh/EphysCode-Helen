% visstimLegFictracEphysOptoVInj.m
%
% Experimental Function
% For simultaneous acquisition of leg video, FicTrac behavior, and 
%  electrophysiology data while providing optogenetic stimulation by 
%  controlling the shutter in front of the mercury lamp for the scope 
%  (light through objective) and visual stimuli on the G3 panels that when
%  in closed loop, are controlled with voltage signals from the DAQ
...
%
% UPDATED:
%   2/22/24 - HHY - modification of visstimLegFictracOptoVInj and
%       visstimLegFictracEphysIInj
%

function [rawData, inputParams, rawOutput] = visstimLegFictracEphysOptoVInj(...
    settings, duration)
    ...
    % name of trial type
    inputParams.exptCond = 'visstimLegFictracEphysOptoVInj';
    ...
    if (oStimTypeFileName == 0)
        ...
        error('No optogenetic stimulation protocol was run. Ending visstimLegFictracEphysOptoVInj()');
    end
    ...
    try
        ...
        % rethrow(errMes);
        error('Invalid optogenetic stimulation function. Ending visstimLegFictracEphysOptoVInj()');
    end
    ...
    if (contains(inputParams.visstimMode, 'closedLoop'))
       if (contains(inputParams.visstimMode, 'X'))
            ...
            if (vInjTypeFileNameX == 0)
                % throw error message; ends run of this function
                error('No voltage injection protocol was run. Ending visstimLegFictracEphysOptoVInj()');
            end
            ...
            try
                ...
                rethrow(errMes);
                % error('Invalid voltage injection function. Ending visstimLegFictracEphysOptoVInj()');
            end
