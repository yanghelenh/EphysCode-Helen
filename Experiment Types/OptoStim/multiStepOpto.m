...
%  lamp for durations specified by user.
% When specifying durations, enter as vector, with square brackets
...
function [optoStimOut, optoStimParams] = multiStepOpto(settings, ...
    durScans)

    % prompt user for input parameters, as dialog box
    inputParams = {'ND filters (total OD):', 'Stim BP filter', ...
        'Stim Durations (s):', 'Duration between Stims (s):', ...
        'Randomize durations? y/n'};
    ...
    allStimDurs = eval(dlgAns{3}); % durations must be entered as valid vector
